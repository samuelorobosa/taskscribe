...
const activateATask = async (id) => {
    ...
    if (__targetIndex !== -1){
        prevTasks[__targetIndex].active = true;
    } else {
    ...
    writeFile('myTask.json', JSON.stringify(prevTasks), 'utf-8', function (err) {
        ...
        console.log( chalk.greenBright.bold(`Task Activated Successfully`));
        process.exit(0);
    })
}

export default activateATask;